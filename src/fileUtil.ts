...
export class FileUtil {
    public static async directoryExists(directory: Directory, path: string): Promise<boolean> {
        try {
            ...
            return statResult.type === "directory" || statResult.type === "NSFileTypeDirectory" as any;
        } catch (error) {
            return false;
        }
    }
    ...
    public static async fileExists(directory: Directory, path: string): Promise<boolean> {
        try {
            ...
            return statResult.type === "file" || statResult.type === "NSFileTypeRegular" as any;
        } catch (error) {
            return false;
        }
    }
        ...
        */
        ...
        if (await FileUtil.directoryExists(destinationDir.directory, destinationDir.path)) {
            ...
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                if (ignoreList.includes(file as unknown as string)) continue;
